...
static metadata_item *getItem(metadata_item *metadata, char *key)
{
    ...
    for (metadata_item *child = metadata->children; child; child = child->next)
    {
...
static void spectrumStrToArr(char *spectrum, SingleSpectrum *s)
{
    ...
    if (debug)
        printf("Allocating Single Spectrum buffer of size %d\n", length);
        ...
        if (strcmp(entry->key, "Spectrum") == 0)
        {
            ...
            if (strstr(id->value, "Coinc") == 0)
            {
                ...
                SingleSpectrum *new_single = (SingleSpectrum*)\
                                             calloc(1, sizeof(SingleSpectrum));
                     ...
                     attr; attr = attr->next)
                {
                    if (strcmp(attr->key, "ChannelData") == 0)
                    {
                        spectrumStrToArr(attr->value, dm->singles[i]);
                                    ...
                                    "radDetectorInformationReference") == 0)
                    {
                        dm->singles[i]->detname = strdup(attr->value);
                    }
                    else if (strcmp(attr->key,
                                    "energyCalibrationReference") == 0)
                    {
