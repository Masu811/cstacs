 ...
 */
void plot(char *headers[], double *columns[], const int n_cols,
          const int n_rows)
{
...
int singleComplete(SingleSpectrum *s)
{
    ...
    if (s->energies == NULL && s->ecal_found == false)
    {
        puts("Single Spectrum is missing energy calibration");
        return 0;
    }
...
void plotSingle(SingleSpectrum *s)
{
    ...
    for (int i = 0; i < n; i++)
        x[i] = (double)s->energies[i];

    double *y = (double*)malloc(n * sizeof(double));

    for (int i = 0; i < n; i++)
        y[i] = (double)s->spectrum[i];

    char *headers[2] = {"Energy", "Counts"};
    double *columns[2] = {x, y};
 ...
 */
int analyze(SingleSpectrum *s, const double s_width, const double w_width,
            const double w_dist, const bool w_rightonly,
            const double peak_width, const double bg_frac, const bool bg_corr,
            double v2p_bounds[4], const int follow_peak_order)
{
    if (!singleComplete(s))
        return 1;
